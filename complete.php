                                    ...
                                    if(mysqli_num_rows($query_run) > 0)
                                    {
                                        foreach($query_run as $student)
                                        {
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="show 3.php?id=<?= $student['ID']; ?>" class="btn btn-info btn-sm">View</a>
                                                    <a href="edit3.php?id=<?= $student['ID']; ?>" class="btn btn-success btn-sm">Edit</a>
													 <form action="code.php" method="POST" class="d-inline">
                                                        <button type="submit" style="background-color:Blue" name="pending" value="<?=$student['ID'];?>" class="btn btn-success btn-sm">Pending</button>
                                                    </form>
                                                    <form action="code.php" method="POST" class="d-inline">
                                                        <button type="submit" name="delete_student3" value="<?=$student['ID'];?>" class="btn btn-danger btn-sm">Delete</button>
                                                    </form>
                                                </td>
                                            </tr>
                                            <?php
